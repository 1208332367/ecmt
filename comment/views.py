		...
		if len(qset)>0:
			...
			if len(qset)>0:
				...
				for i in range(0, len(DATA)):
					...
					tmp['date'] = tmp['mtime'][0:10]
					tmp['time'] = tmp['mtime'][11:16]
...
def favor_comment(request):
	...
	try:
		...
		if len(qset) == 0:
			...
			if current_favor>1:
				tag = -1
				res = {'code': -1, 'msg': '您已为该评论点赞，不能重复点赞！', 'data': []}
			if current_oppose>1:
				tag = -1
				res = {'code': -1, 'msg': '您已反对该评论一次，不能重复反对！', 'data': []}
	...
	try:
		...
		if len(qset)>0:
			...
			for i in range(0,len(DATA)):
				...
				tmp['date'] = tmp['mtime'][0:10]
				tmp['time'] = tmp['mtime'][11:16]
				qset2 = dynamics.objects.filter(comment_id=DATA[i]['pk'],status=1)
				if len(qset2)>0:
					tag = 0
					DATA2 = json.loads(serializers.serialize("json", qset2))
					for j in range(0,len(DATA2)):
